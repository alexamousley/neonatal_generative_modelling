...
# This script takes the output of the calculate_organization_measures.m code for OBSERVED network measures and runs
...
# Load network data
density <- readMat('example_density.mat')
density <- as.data.frame(density$density)
global_statistics <- readMat('example_observed_global_statistics.mat')
global_statistics <- as.data.frame(global_statistics$example.observed.global.statistics)
local_statistics <- readMat('example_observed_local_statistics.mat')
# Load each local statistic
degree <- as.data.frame(local_statistics$example.observed.local.statistics[,,1])
betweenness <- as.data.frame(local_statistics$example.observed.local.statistics[,,2])
clustering <- as.data.frame(local_statistics$example.observed.local.statistics[,,3])
edge_length <- as.data.frame(local_statistics$example.observed.local.statistics[,,4])
local_efficiency <- as.data.frame(local_statistics$example.observed.local.statistics[,,5])
matching <- as.data.frame(local_statistics$example.observed.local.statistics[,,6])
...
# Run GAM
density_model <- gam(density ~ s(postconceptional_age,bs="cr",k=3)+s(birth_age,bs="cr",k=3)
...
model_cs <- gam(density ~ s(postconceptional_age,bs="cs",k=3)+s(birth_age,bs="cs",k=3)
                +sex+head_circum+translation+rotation, data=data,method='REML')
...
density_plot <- ggplot() +
  ...
  geom_polygon(fill = "gray85", data = reg_plots[[1]]$layers[[1]]$data, aes(x = x, y = y * 100), alpha = 0.9) +
  ...
  geom_polygon(fill = "gray85", data = reg_plots[[2]]$layers[[1]]$data, aes(x = x, y = y * 100), alpha = 0.9) +
...
### Modularity GAM ###
modularity_model <- gam(modularity ~ s(postconceptional_age,bs="cr",k=3)+s(birth_age,bs="cr",k=3)
                     +sex+head_circum+translation+rotation, data=data,method='REML')
...
# Determine if wiggle is needed (via cs basis function)
model_cs <- gam(modularity ~ s(postconceptional_age,bs="cs",k=3)+s(birth_age,bs="cs",k=3)
                +sex+head_circum+translation+rotation, data=data,method='REML')
p <- par(mfrow=c(2,2))    # Plot and visually inspet if model_cs reduced wiggle
...
global_efficiency_model <- gam(global_efficiency ~ s(postconceptional_age,bs="cr",k=3)+s(birth_age,bs="cr",k=3)
                        +sex+head_circum+translation+rotation, data=data,method='REML')
...
### Path Length GAM ###
path_length_model <- gam(characteristic_path_length ~ s(postconceptional_age,bs="cr",k=3)+s(birth_age,bs="cr",k=3)
                               +sex+head_circum+translation+rotation, data=data,method='REML')
...
rich_connections_model <- gam(rich_connections ~ s(postconceptional_age,bs="cr",k=3)+s(birth_age,bs="cr",k=3)
                               +sex+head_circum+translation+rotation, data=data,method='REML')
...
rich_lengths_model <- gam(rich_lengths ~ s(postconceptional_age,bs="cr",k=3)+s(birth_age,bs="cr",k=3)
                              +sex+head_circum+translation+rotation, data=data,method='REML')
...
# Run model
rich_prop_model <- gam(rich_proportion ~ s(postconceptional_age,bs="cr",k=3)+s(birth_age,bs="cr",k=3)
