...
# This script runs generalized additive models of the model fit/parameters as well as exploring network organization
...
head_circum <- readMat('example_head_circumference.mat')
head_circum <- as.data.frame(head_circum$example.head.circumference)
translation <- readMat('example_translation.mat')
translation <- as.data.frame(translation$example.translation)
rotation <- readMat('example_rotation.mat')
rotation <- as.data.frame(rotation$example.rotation)
sex <- readMat('example_sex.mat')
sex <- as.data.frame(sex$example.sex)

# Load network data
# Observed
obs_global_statistics <- readMat('example_observed_global_statistics.mat')
obs_global_statistics <- as.data.frame(obs_global_statistics$example.observed.global.statistics)
obs_local_statistics <- readMat('example_observed_local_statistics.mat')
# Load each local statistic
obs_degree <- as.data.frame(obs_local_statistics$example.observed.local.statistics[,,1])
obs_betweenness <- as.data.frame(obs_local_statistics$example.observed.local.statistics[,,2])
obs_clustering <- as.data.frame(obs_local_statistics$example.observed.local.statistics[,,3])
obs_edge_length <- as.data.frame(obs_local_statistics$example.observed.local.statistics[,,4])
obs_local_efficiency <- as.data.frame(obs_local_statistics$example.observed.local.statistics[,,5])
obs_matching <- as.data.frame(obs_local_statistics$example.observed.local.statistics[,,6])
# Simulated
sim_global_statistics <- readMat('example_simulated_global_statistics.mat')
sim_global_statistics <- as.data.frame(sim_global_statistics$example.simulated.global.statistics)
sim_local_statistics <- readMat('example_simulated_local_statistics.mat')
# Load each local statistic
sim_degree <- as.data.frame(sim_local_statistics$example.simulated.local.statistics[,,1])
sim_betweenness <- as.data.frame(sim_local_statistics$example.simulated.local.statistics[,,2])
sim_clustering <- as.data.frame(sim_local_statistics$example.simulated.local.statistics[,,3])
sim_edge_length <- as.data.frame(sim_local_statistics$example.simulated.local.statistics[,,4])
sim_local_efficiency <- as.data.frame(sim_local_statistics$example.simulated.local.statistics[,,5])
sim_matching <- as.data.frame(sim_local_statistics$example.simulated.local.statistics[,,6])

# Load model fit data
energy <- readMat('example_energy_sorted.mat')
...
params <- readMat('example_mean_top_params.mat')
params <- as.data.frame(params$mean.top.params)
TFdissimilarity <- readMat('example_TFdissimilarity.mat')
...
### Energy GAM ###
energy_model <- gam(energy ~ s(postconceptional_age,bs="cr",k=3)+s(birth_age,bs="cr",k=3)
...
energy_plot <- ggplot() +
  ...
  geom_polygon(fill = "gray85", data = reg_plots[[1]]$layers[[1]]$data, aes(x = x, y = y), alpha = 0.9) +
  ...
  geom_polygon(fill = "gray85", data = reg_plots[[2]]$layers[[1]]$data, aes(x = x, y = y), alpha = 0.9) +
...
### Eta GAM ###
eta_model <- gam(eta ~ s(postconceptional_age,bs="cr",k=3)+s(birth_age,bs="cr",k=3)
                               +sex+head_circum+translation+rotation, data=data,method='REML')
...
### Gamma GAM ###
gamma_model <- gam(gamma ~ s(postconceptional_age,bs="cr",k=3)+s(birth_age,bs="cr",k=3)
                              +sex+head_circum+translation+rotation, data=data,method='REML')
