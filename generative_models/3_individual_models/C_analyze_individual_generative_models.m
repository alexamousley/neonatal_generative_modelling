...
load('generative_models.mat');
% Load atlas euclidean distances
load('euclidean_distances.mat');
% Load birth age
load('example_birth_age.mat');
term_index = birth_age>=37; % Define which participants were born at term
% Load observed organizational measures
load('example_observed_global_statistics.mat')
load('example_observed_local_statistics.mat');
...
Asynth_sorted = cell(size(generative_models.networks)); 
Eall_sorted = zeros(size(generative_models.energy));

% Loop over each participant
for sub = 1:nsub
    h = squeeze(generative_models.energy(sub,:));                    % Select energy for subject for model type
    [~,i] = sort(h);                                                 % Create index of acending order of energy
    Eall_sorted(sub,:) = generative_models.energy(sub,i);            % Sort energy
    params_sorted(sub,:,:) = generative_models.parameters(sub,i,:);  % Sort parameters by energy index
    Asynth_sorted{sub} = generative_models.networks{sub}(:,i);       % Sort generative models by energy index
...
% Set up variables
emean = mean(generative_models.energy);           % Take mean energy 
p = squeeze(generative_models.parameters(1,:,:)); % Define parameters
...
for sub  = 1:nsub
    ...
    simulated_global_statistics(sub,9) = mean(con_length(find(con_length(:,2)>0),2));
end
...
% Take the statistics
obs = mean(squeeze(example_observed_local_statistics(:,:,measure)))';
...
for sub = 1:nsub
    ...
    observed = squeeze(example_observed_local_statistics(sub,:,:));
