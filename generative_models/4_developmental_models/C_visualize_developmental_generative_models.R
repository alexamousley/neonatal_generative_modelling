...
term_lengths <- readMat('example_network_term_lengths.mat')
term_lengths <- as.data.frame(t(term_lengths$network.term.lengths))
preterm_lengths <- readMat('example_network_preterm_lengths.mat')
preterm_lengths <- as.data.frame(t(preterm_lengths$network.preterm.lengths))
term_lengths_per_type <- readMat('example_mean_term_connection_type_lengths.mat')
term_lengths_per_type <- as.data.frame(term_lengths_per_type$mean.term.connection.type.lengths)
preterm_lengths_per_type <- readMat('example_mean_preterm_connection_type_lengths.mat')
preterm_lengths_per_type <- as.data.frame(preterm_lengths_per_type$mean.preterm.connection.type.lengths)

# Create iteration and group variables
iteration <- as.data.frame(rep(1:nrow(preterm_counts)))
group <- c(rep(1,406),rep(2,406))
...
for (i in 1:length(models)) {
  models[[i]][is.nan(models[[i]])] <- 0
}
for (i in 1:length(grouped_models)) {
  grouped_models[[i]][is.nan(grouped_models[[i]])] <- 0
}
...
# Plot (Figure 6C)
length_plot <- ggplot()+
  geom_polygon(fill="gray85",aes(x=term_reg_plots$layers[[1]]$data$x,y=term_reg_plots$layers[[1]]$data$y),alpha=0.9)+
  geom_line(aes(x=term_reg_plots$layers[[3]]$data$x, y=term_reg_plots$layers[[3]]$data$y),colour='#02c0d7',linewidth=4)+
  geom_point(aes(x=term_reg_plots$data$x, y=term_reg_plots$data$y),colour='#02c0d7',alpha=0.5)+
  geom_polygon(fill="gray85",aes(x=preterm_reg_plots$layers[[1]]$data$x,y=preterm_reg_plots$layers[[1]]$data$y),alpha=0.9)+
  geom_line(aes(x=preterm_reg_plots$layers[[3]]$data$x, y=preterm_reg_plots$layers[[3]]$data$y),colour="#2d3063",linewidth=4)+
  geom_point(aes(x=term_reg_plots$data$x, y=term_reg_plots$data$y),colour="#2d3063",alpha=0.5)+
  theme_classic() + 
  theme(text=element_text(family="Arial",size=60),axis.title=element_blank())
