...
load('example_initial_generative_models.mat');
...
nruns = length(output.params);
% Define size of network
nsize = length(output.networks);
...
Eall_sorted = zeros(size(output.energy));   % Energies

for model = 1:nmodels
    modelE = squeeze(output.energy(model,:));                      % Select energy for model type
    [~,modelI] = sort(modelE);                                     % Create index of that sorts energy (from lowest to highest)
    Eall_sorted(model,:) = output.energy(model,modelI);            % Create a new variable of energy that is sorted 
    params_sorted(model,:,:) = output.params(model,modelI,:,:);    % Sort parameters by energy index
    Asynth_sorted(model,:,:)= output.networks(model,:,modelI);     % Sort generative models by energy index
end 
...
for model = 1:nmodels
    ...
    e = squeeze(output.energy(model,:));
    % Create landscape plot
    subplot(4,4,model);
    scatter(output.params(model,:,1),output.params(model,:,2),400,e,'.');
...
figure(2);clf(2);
scatter(output.params(model,:,1),output.params(model,:,2),100,e,'.'); 
...
for model = 1:nmodels
    ...
    observed_network = example_consensus_network; 
