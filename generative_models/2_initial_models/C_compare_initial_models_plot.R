...
energies <- readMat('example_initial_model_energies.mat')
energies <- as.data.frame(t(energies$Eall.sorted))   # This variable will be runs x model
top_energies <- energies[1:10,]                      # Select top 10 model energies for each model type
...
# Reshape energy
energies_melt <- melt(top_energies)
